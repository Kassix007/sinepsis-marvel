			...
			this.time.addEvent({
				...
				callback: () => {
					if (this.game_over_timeout === 0) {
						...
						this.scene.start("GameOverScene", { points: this.points, time: `Time left: 0s` });
					} else if (this.points >= 10) {
						// Points threshold reached
						this.game.events.removeListener("start-game");
						this.scene.stop("HudScene");
						this.scene.start("GameOverScene", { points: this.points, time: `Time left: ${this.game_over_timeout}s` });
					} else {
